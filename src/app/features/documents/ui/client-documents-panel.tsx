...
type ClientDocumentsPanelLabels = {
  title: string
  ...
  categoriesStatusTitle: string
  categoryPendingLabel: string
  categoryUploadedSingularLabel: string
  categoryUploadedPluralLabel: string
  emptyList: string
...
type ClientDocumentsPanelProps = {
  documentsPageHref: string
  canManageCategories: boolean
  initialCategories: CategoryItem[]
  initialCategorySummaries: CategorySummaryItem[]
  documentsCount: number
  labels: ClientDocumentsPanelLabels
}
...
export function ClientDocumentsPanel({
  ...
  const [categoryDocumentCounts, setCategoryDocumentCounts] = useState<Record<string, number>>(
    Object.fromEntries(initialCategorySummaries.map((item) => [item.categoryPublicId, item.documentsCount])),
  )
  ...
  const countLabel = documentsCount === 1 ? labels.countLabelSingular : labels.countLabelPlural
  ...
  async function handleCreateCategory(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await apiRequest<{ category: CategoryItem }>("/api/document-categories", {
        method: "POST",
        body: { name: categoryName },
      })

      setCategories((current) => [...current, response.category])
      setCategoryDocumentCounts((current) => ({
        ...current,
        [response.category.publicId]: 0,
      }))
      setCategoryName("")
  ...
  async function handleUpdateCategory(category: CategoryItem) {
    ...
    try {
      const response = await apiRequest<{ category: CategoryItem }>(
      ...
      setCategories((current) =>
        current.map((item) => (item.publicId === category.publicId ? response.category : item)),
      )
    } catch {
      showToast({
        title: labels.categoryCreateErrorTitle,
  ...
  function toCategoryStatus(documentsInCategory: number) {
    if (documentsInCategory === 0) {
      return labels.categoryPendingLabel
    }

    return documentsInCategory === 1
      ? `1 ${labels.categoryUploadedSingularLabel}`
      : `${documentsInCategory} ${labels.categoryUploadedPluralLabel}`
  }
  ...
  return (
    <article className="clients-detail-card">
      ...
      <p className="clients-detail-card__count">
        {documentsCount} {countLabel}
      </p>
      <p>{labels.description}</p>

      {sortedCategories.length > 0 ? (
        <div className="documents-panel__summary-wrap">
          <h3>{labels.categoriesStatusTitle}</h3>
          <ul className="documents-panel__summary-list">
            {sortedCategories.map((category) => {
              const docsInCategory = categoryDocumentCounts[category.publicId] ?? 0
              const hasDocuments = docsInCategory > 0

              return (
                <li key={category.publicId}>
                  <strong>{category.name}</strong>
                  <span className={hasDocuments ? "documents-panel__summary-status--ok" : ""}>
                    {toCategoryStatus(docsInCategory)}
                  </span>
                </li>
              )
            })}
      ...
      {canManageCategories ? (
        <div className="documents-panel__category-actions">
      ...
      <div className="documents-panel__view-action">
        <Link href={documentsPageHref} className="clients-secondary-link">
          {labels.viewDocumentsButton}
        </Link>
      </div>

      {documentsCount === 0 ? <p className="clients-payment-list__empty">{labels.emptyList}</p> : null}

      {canManageCategories && isCreateCategoryModalOpen ? (
        <div className="modal-overlay" role="dialog" aria-modal="true" aria-labelledby="category-create-title">
          <div className="modal-card clients-delete-modal">
            <h2 id="category-create-title">{labels.categoryCreateButton}</h2>

            <form onSubmit={handleCreateCategory} className="documents-panel__modal-form">
              <label htmlFor="document-category-name">{labels.categoryNameLabel}</label>
              <input
      ...
      {canManageCategories && isViewCategoriesModalOpen ? (
        <div className="modal-overlay" role="dialog" aria-modal="true" aria-labelledby="categories-view-title">
          <div className="modal-card documents-categories-modal">
            <h2 id="categories-view-title">{labels.categoriesListTitle}</h2>
      ...
      {categoryToDelete ? (
        <div className="modal-overlay" role="dialog" aria-modal="true" aria-labelledby="category-delete-title">
          <div className="modal-card clients-delete-modal">
            <h2 id="category-delete-title">{labels.categoryDeleteModalTitle}</h2>
            <p>
